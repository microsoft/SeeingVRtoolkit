...
Shader "Hidden/EdgeDetectionCustomized" {
	Properties{
...
#include "UnityCG.cginc"
	...
	half4 fragRobert(v2f i) : SV_Target
	{
		...
		if (unity_StereoEyeIndex == 0) {
			if (length(i.uv[0] - UnityStereoScreenSpaceUVAdjust(float2(0.56, 0.5), _MainTex_ST)) < 0.2)
				return tex2D(_MainTex, i.uv[0]);
		}
		else {
			if (length(i.uv[0] - UnityStereoScreenSpaceUVAdjust(float2(0.5, 0.5), _MainTex_ST)) < 0.2)
				return tex2D(_MainTex, i.uv[0]);
		}
		...
		if (edge == 0) return float4(0, 1, 0, 1);
		return tex2D(_MainTex, i.uv[0]);
	}
